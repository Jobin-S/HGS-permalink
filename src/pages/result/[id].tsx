...
import { outcomes as outComeArray } from '@/constants/test';
...
export default function Page({permalink = true}:Props) {
  const router = useRouter()
  const [outcome, setOutCome] = useState<OutcomeType>()
  const [isShare, setIsShare] = useState(false);
const {getResultCardImage,GACustomEvent} = useUtils()
const [showResult, setShowResult] = useState(false)

const url = "https://www.joinhgs.com/" ;
const text =
  "Indulge in a space adventure to find a tech role based on your personality \nClick the link below to take the quiz.";
...
const bgSpotColor = "rgba(99, 99, 99, 0.35)";
  ...
  const shareOnTwitter = () => {
    GACustomEvent("Share", "techXplorers/Twitter");
  ...
  const shareOnLinkedIn = () => {
    GACustomEvent("Share", "techXplorers/Linkedin");
  ...
  const shareOnFacebook = () => {
    GACustomEvent("Share", "techXplorers/Facebook");
    window.open(
  ...
  return(
   ...
   <div>
    <Head>
    <title>{outcome?.code} - HGS</title>
        <meta property="og:image" content={`/images/og/${outcome?.code}.png`} />
        <meta property="og:url" content={`./${outcome?.code}`} />
    </Head>
     <div
    ...
    >
      ...
      <div className="share_btn_con">
        ...
        >
          {permalink ? "View your result" : "Apply Now"}
        </button>
        {!permalink && (
          <div
